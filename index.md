...
1. For every **5 DR**, add one **d12** to the pool.  
2. If any DR remains (1–4), add one die based on the remainder:
   - **1 DR → d4**
   - **2 DR → d6**
   - **3 DR → d8**
   - **4 DR → d10**

Dice Pools are written like this:

**`{d12, d8}kh`** → roll all dice in the braces; **keep highest** (with special 7+ rule).
...
1. Roll **d20**.  
2. Roll your DR die or full Dice Pool.  
3. Identify the **single highest** die result.  
4. Add together:
   - d20 result  
   - Trait bonus or penalty  
   - Highest DR / Dice Pool result  
...
- **Any 7+ dice** also add to the total and negate Nat1.
...
Non-player characters, creatures, and animals use a compact, line-based stat block. 
Each line has a specific purpose and is always presented in the same order.
...
- Modifiers may be positive or negative. Usually -2 to +4. 5+ exceptions are possible with Prowess for more brutish, giant beasts.

**Line 3 — Defense & Survival**  
Lists core defensive values and durability.
- Defense (Def) See Armor Rules
- Armor type (descriptive of Def bonus and body armor)
- Armor Save (Save) See Armor Rules
- Vigor (NPC vitality) See Glory Points vitality rules

**Line 4+ — Attacks**  
Each attack is listed on its own line, prefixed with a dash (`-`).
- Attack Name
- To-Hit (Trait+DR) bonus
- Damage die or dice
...
- **Parry+7 +3DR**
Conversion: Unique, only if gravely wounded, healed, and shown compassion. May later guard a church.

## One-Line Creature Template

**Name/Role: Pro_, Awa_, Sav_, Spi_, Int_, Soc_ | Def__, *armor*, Save___, Vigor___ | Attack(Trait+DR), damage_____ | Skill(Trait+DR)**
...
- **Attack (Trait+DR)________, damage__________**  
**Abilities: (Trait+DR) __________, __________**
